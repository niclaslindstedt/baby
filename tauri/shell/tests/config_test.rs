...
//!
//! Both matter more than their size suggests: the origin is what every record
//! on the machine is keyed to, and the navigation guard is the whole of this
//! shell's security policy.

use baby_shell::config::{app_origin, is_internal_url, start_url, APP_HOST, APP_SCHEME};
...
fn each_platform_gets_the_origin_it_grants() {
    assert_eq!(app_origin(false), "baby://localhost");
    assert_eq!(app_origin(true), "http://baby.localhost");
}

/// Not a tautology: these two words are what every stored record is keyed to,
...
fn the_origin_is_built_from_the_two_constants() {
    assert_eq!(APP_SCHEME, "baby");
    assert_eq!(APP_HOST, "localhost");
}

#[test]
fn the_window_opens_on_the_entry_page() {
    assert_eq!(start_url("baby://localhost"), "baby://localhost/index.html");
    // A trailing slash on the origin must not become a double one.
    assert_eq!(
        start_url("baby://localhost/"),
        "baby://localhost/index.html"
    );
}

#[test]
fn our_own_pages_navigate_in_the_window() {
    let origin = "baby://localhost";
    assert!(is_internal_url("baby://localhost", origin, None));
    assert!(is_internal_url("baby://localhost/index.html", origin, None));
    assert!(is_internal_url("baby://localhost/privacy/", origin, None));
}
...
fn anything_else_does_not() {
    let origin = "baby://localhost";
    for outside in [
        "https://github.com/niclaslindstedt/baby",
        "https://baby.niclaslindstedt.se/",
        "file:///etc/passwd",
        // The near-miss that a naive `starts_with` on the scheme would admit.
        "baby://localhost.example.com/",
    ] {
        assert!(
...
/// A launch pointed at a remote build (`BABY_APP_URL`) navigates within THAT
/// site too — otherwise every in-app link would bounce to the browser.
#[test]
fn a_remote_launch_navigates_within_its_own_site() {
    let origin = "baby://localhost";
    let remote = Some("https://baby.niclaslindstedt.se/preview/");
    assert!(is_internal_url(
        "https://baby.niclaslindstedt.se/preview/index.html",
        origin,
        remote
    ));
