...
//! shipping: `BABY_APP_URL` points the window at a remote URL instead (the
//! `/preview/` deploy slot, say), and `BABY_WEBROOT` serves a different
...
/// IndexedDB unable to keep a record between launches. A registered scheme
/// gives one stable origin that the records are keyed to for the life of the
/// install.
pub const APP_SCHEME: &str = "baby";
...
/// CONSTANT — the origin is what the user's records are keyed to, so changing
...
/// The app's dark surface (`BG` in `scripts/generate-icons.mjs`, `#12101a`),
/// painted behind the page so no white flash shows through while it loads.
pub const BRAND_BG: (u8, u8, u8, u8) = (0x12, 0x10, 0x1a, 0xff);

/// What the window is called.
pub const WINDOW_TITLE: &str = "Baby";

/// The window's opening size, and the smallest it may be dragged to.
///
/// The floor is the narrowest the day's list is still usable at — below it
