...
//! wrong to simplify.
//!
//! The site is served from a registered private scheme (`baby://`, see
...
//! to lose:
//!
//!  1. **One stable origin.** The user's records live in IndexedDB and their
//!     settings in `localStorage`, both keyed by origin. `baby://` is a
//!     constant, so the records survive every update. A `file://` page (or a
//!     server on an ephemeral port) would hand the user a different origin —
//!     and an empty records — at some point.
//!
...
/// `baby:///C:/…` must not become a drive letter.
