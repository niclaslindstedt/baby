...
//! The Baby desktop shell — a window showing the bundled site, and nothing
//! else.
//!
...
//! here needed no change to the app at all. Everything Baby does — the
...
use baby_shell::config::APP_SCHEME;
use baby_shell::oauth::{AWAIT_PATH, BEGIN_PATH};
use baby_shell::webroot::webroot_exists;
...
fn main() {
    tauri::Builder::default()
        ...
        // `baby_shell::webroot` for the three properties this arrangement is
        ...
        // `baby_shell::oauth`), which can be minutes, so it answers from a
        ...
        .setup(|app| {
            ...
            if baby_shell::config::remote_app_url().is_none() && !webroot_exists(&root) {
                let message = format!(
                    "Baby could not find the app it is meant to show.\n\n\
                     Looked in: {}\n\n\
        ...
        .expect("the Baby desktop shell could not start");
}
