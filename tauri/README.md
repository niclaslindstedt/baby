...
# Baby — the desktop app

A desktop wrapper around Baby for **Windows, macOS and Linux**. It is a thin
...
minimum. Everything Baby does — the document, the settings, the sync engine —
...
| `src-tauri/src/protocol.rs` | Answering `baby://` off the bundled `webroot/`                |
...
`cargo test -p baby-shell` runs the entire decision layer on a machine with a
...
**The origin is the one thing to be careful with.** The user's records live in
...
`file://` page is an opaque origin, which would leave the records unable to
survive a launch.
...
| Variable       | Effect                                                               |
| -------------- | -------------------------------------------------------------------- |
| `BABY_APP_URL` | Load a remote URL instead of the bundled site (the `/preview/` slot) |
| `BABY_WEBROOT` | Serve the site from somewhere else without rebuilding                |
...
identifier (`dev.local.baby`). What an installed copy is called and the
