...
// the session round-trip through it, so logging a diaper, adding a reading, and
// watching the screens move all behave exactly as they do against the real
 ...
 * was seeded on, so a session that runs past midnight keeps the history the
...
export function createDemoBackend(): DocBackend {
  ...
  return {
    ...
    save(next) {
      ...
      // touched. It cannot fail, so demo taps confirm exactly as real ones
      // do.
      doc = next;
      return true;
    },
