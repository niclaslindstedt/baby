...
// `migrations.ts`'s and `merge.ts`'s, exactly as they are for Dropbox and for
// the picked folder — which is why the host is adapted into
 ...
 *  a revision history, a notion of what a weighing is — would be a host
 *  holding a second copy of this app's domain. */
...
export const CLOUD_HOST_EVENT = "baby:cloud-host";
...
export const CLOUD_HOST_PROPERTY = "__babyCloudHost";
...
function unwrap(result: CloudHostResult): unknown {
  ...
  // `undefined` and, one push later, an empty file where somebody's record
  // were.
