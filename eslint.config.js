...
export default [
  ...
  js.configs.recommended,
  {
    ...
    files: [
      ...
      "tauri/scripts/**/*.mjs",
      ".agents/skills/**/*.mjs",
      // The native wrapper's Node-side JavaScript: its Expo config, its
      // identity module, its Metro config and its bundle script.
      "native/**/*.{js,mjs}",
    ],
    languageOptions: {
      sourceType: "module",
  ...
  },
  {
    files: [
      "src/**/*.{ts,tsx}",
      "tests/**/*.{ts,tsx}",
      // The native wrapper's app sources. Linted from here so there is one set
      // of rules for the repo; `native/` has its own dependency tree and its
      // own `tsc` (`npm --prefix native run typecheck`), which is what
