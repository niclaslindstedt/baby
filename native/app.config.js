...
// has no version of its own — it ships one build of the baby log, and the
...
module.exports = () => ({
  expo: {
    ...
    slug: "baby",
    version,
    // A one-column log, read and filled in one-handed — often with the other
    // hand holding the baby. Landscape is not broken, but it is not what the
    // layout is measured for, so the app is pinned the way the phone is held.
    ...
    // `se.agilator.baby` in a store build and `dev.local.baby` in a plain
    ...
    ios: {
      ...
      infoPlist: {
        ...
        // folder called "Baby", so the parent can see, copy and back up the
        // record the app keeps there. Without this the container syncs but is
        // invisible — a record its owner cannot open.
        NSUbiquitousContainers: {
          [ICLOUD_CONTAINER]: {
            NSUbiquitousContainerIsDocumentScopePublic: true,
            NSUbiquitousContainerSupportedFolderLevels: "None",
            NSUbiquitousContainerName: "Baby",
          },
        },
    ...
    extra: {
      // NO remote URL here, deliberately. The app serves the copy of the baby
      ...
      // debug build at a deployed slot, set EXPO_PUBLIC_BABY_URL at build
