...
devices' edits reconcile, are the web app's, in `src/app/migrations.ts` and
`merge.ts`.
...
```
Settings → Where the record lives → Dropbox
...
window.__ossAuthSession.open(authorizeUrl)   — installed by src/authSessionBridge.ts
   ...
   ▼
App.tsx → src/authSession.ts → WebBrowser.openAuthSessionAsync(url, "se.agilator.baby://oauth")
   │  the reader consents in the sheet; Dropbox redirects to
   │  se.agilator.baby://oauth?code=…&state=dropbox and the sheet closes
   ▼
...
**the redirect URI is `se.agilator.baby://oauth`** in a store build (`dev.local.baby://oauth`
in a plain checkout). Dropbox requires the exact URI to be registered, so the
Dropbox app behind `VITE_DROPBOX_APP_KEY` must list `se.agilator.baby://oauth` under
...
- **The port in `src/local-server.ts` is fixed on purpose.** A web origin is
  ...
  port would hand the WebView an empty store on every launch, and everything
  the parent logged would appear to vanish.
