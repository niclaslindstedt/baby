 ...
 * would come back to an empty container and the child's record would appear to
 * be gone (it is not; it is in the old container, and unreachable). It is
 * pinned in three places that must agree: here, `../../identifiers.js` (which
 * `app.config.js` reads for the iCloud entitlements and the Files-app
 * declaration), and `ios/ICloudStoreModule.swift`.
 */
export const ICLOUD_CONTAINER = "iCloud.se.agilator.baby";
 ...
 * the file holding their child's record. A health log that syncs to a place
 * its owner cannot open would be a worse answer than not syncing at all.
 */
