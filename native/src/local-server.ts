...
// store: everything the parent has logged would appear to vanish. The port is
 ...
 *  the first one is what keeps the user's record across launches.
 *
 ...
 *  checklist 8791, the games 9006 / 9007 / 9033 — so no two contend for a port
 *  on a phone that has both. A new wrapper takes the next free ten. */
const PORT_LADDER = [8301, 8302, 8303] as const;
...
export type LocalServer = {
  /** The origin to point the WebView at, e.g. `http://localhost:8301`. */
  origin: string;
  stop: () => Promise<void>;
};
