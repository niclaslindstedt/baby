...
// `http://localhost:<port>…`: an origin it does not have registered, in a
...
export const AUTH_SESSION_REQUEST_TYPE = "baby-native/auth-session-request";
 ...
 * is `app.config.js`'s `scheme` — the bundle id, so `se.agilator.baby://oauth`
 * in a store build. This exact string is what the Dropbox app's App Console
