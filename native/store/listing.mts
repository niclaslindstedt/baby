...
export const RULES: StoreRules = {
  ...
  brand: {
    ...
    // behaviour does.
    privacyUrl: "https://apps.agilator.se/baby/privacy/",
    supportUrl: "https://apps.agilator.se/baby/support/",
  },

  // Baby ships on the App Store alone. The desktop shell exists (`tauri/`)
  // and ships as a download from the releases page, but no Mac App Store
  // record does, and there is no Steam page — so neither is compiled, and the
  // preflight does not ask for screenshots nobody submits. Turning one on is
  // this flag plus the section it belongs to below.
  storefronts: {
    appStore: true,
    macAppStore: false,
    steam: false,
  ...
  apple: {
    ...
    advisory: {
      ...
      // The one that is not NONE, and honestly so. The question asks what the
      // APP provides, and this one provides health reference information of
      // its own: growth plotted against the WHO standard curves, a check of
      // the day's regimen against the nutrition the baby needs, and the
      // Swedish childhood vaccination schedule. It is reference, not
      // treatment — no dosages, no diagnosis, no advice to act on — so it is
      // the mild end, not the intense one.
      ...
      // Not a Kids-category app, and deliberately: it is FOR the parent, about
      // the child. The child never uses it, and the Kids category's rules are
      // written for apps a child holds.
      kidsAgeBand: null,
    },
