...
channel is worth mentioning at the next visit, and the screen says which way
and by how much.

Ahead of the last reading the chart shades **where the next readings are
likely to land**: a projection that follows your child's own channel, with
...
current length channel as far as length at that age predicts adult height
and regresses the rest of the way toward the parents' target. How the placing, the trend,
the projection and the target are computed is in
[the growth documentation](../growth.md).
