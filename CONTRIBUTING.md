...
`npx vitest run tests/growth_test.ts`. UI changes should keep the boot smoke
path working: `npm run build && npm run preview`, then set up a child, log a
diaper on Today, add a reading under Growth, and check that the Today cards
move with it.

The domain logic is deliberately clock-free — `today` is a parameter, never
`new Date()` inside the derivation — so a test never needs fake timers. Keep it
that way: a date-dependent test that passes today and fails in March is worse
than no test.
