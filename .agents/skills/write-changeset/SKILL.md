...
| ---------------------------------------------------------------- | ------------------------------------------------------ |
| `src/app/*Screen.tsx`, `BottomNav.tsx`, `DiaperSheet.tsx`        | Yes — the UI moved                                     |
| `src/app/growth.ts`, `nutrition.ts`, `diapers.ts`, `vaccines.ts` | Yes — the numbers or the schedule a user reads changed |
| `src/app/data/*`                                                 | Yes — the standards, presets or programme changed      |
| `src/app/useSyncEngine.ts`, `idbAdapter.ts`, `merge.ts`          | Yes — storage behaviour is user-visible                |
| `src/app/types.ts`, `migrations.ts`                              | Yes, and consider `breaking: true`                     |
| `src/app/i18n/en.ts`, `sv.ts` alone                              | Only if the wording change alters meaning              |
| `tests/`, `docs/`, `.github/`, config                            | No                                                     |

## Update checklist

- [ ] Write the fragment (see the shape below)
- [ ] Title is a short noun phrase, not a commit subject ("Diaper floors by age", not "feat: add diaper norms")
