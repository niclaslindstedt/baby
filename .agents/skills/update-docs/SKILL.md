...
4. Pay special attention to numbers and thresholds quoted in prose: the diaper floors per age band and the breastfed 14-day gap, the energy per kilogram and the breast-milk energy shares, the iron / vitamin D / fat targets per age, the 90% tolerance, the forecast half-life, decay and drift constants, the trend span (60 days) and threshold (0.67 SD), the target-height regression and its ±10 cm, the programme ages, the six-month and four-month stage boundaries, the debounce interval. These are the values most likely to be edited in code and forgotten in prose.

## Mapping

| Changed source                                          | Doc to update                                                                                        |
| ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `src/app/growth.ts`, `src/app/data/whoGrowth.ts`        | `docs/growth.md`; `docs/features/growth.md`; the README's Examples block if the returned shape moved |
| `src/app/nutrition.ts`, `src/app/data/foods.ts`         | `docs/nutrition.md`; `docs/features/food.md`                                                         |
| `src/app/diapers.ts`                                    | `docs/diapers.md`; `docs/features/diapers.md`                                                        |
| `src/app/vaccines.ts`                                   | `docs/vaccinations.md`; `docs/features/vaccinations.md`                                              |
| `src/app/merge.ts`, `useSyncEngine.ts`, `idbAdapter.ts` | `docs/sync.md`; `docs/features/cloud-sync.md`                                                        |
...
| `src/vite-env.d.ts`, `vite.config.ts`, `.env.example`   | `docs/configuration.md` (build-time table); the README's Configuration table                         |
| `src/app/*Screen.tsx`, `BottomNav.tsx`, `TopBar.tsx`    | The matching `docs/features/*.md`; `docs/architecture.md` (the shell); the README's Usage table      |
