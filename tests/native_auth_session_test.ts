...
const REDIRECT = redirectUriFor("se.agilator.baby");
...
describe("the redirect URI", () => {
  it("is the app's scheme with an oauth path — the string Dropbox must list", () => {
    // The scheme is the bundle id (tests/native_scheme_test.ts), so a store
    // build comes back on this exact URI.
    expect(REDIRECT).toBe("se.agilator.baby://oauth");
  });
});
...
describe("isAuthSessionRequest", () => {
  ...
  it("ignores messages that are not ours", () => {
    expect(isAuthSessionRequest({ ...good, type: "baby-native/theme" })).toBe(
      false,
    );
...
describe("authSessionResolveScript", () => {
  it("does not let a provider-controlled URL break out of the script", () => {
    ...
    const nasty = `se.agilator.baby://oauth?code=");alert(1);//&x=${String.fromCharCode(0x2028)}`;
